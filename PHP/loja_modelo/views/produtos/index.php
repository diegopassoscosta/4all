    ...
    if($_REQUEST['nome'] != ''){
    	$sql.= "      AND p.nome LIKE '%" . $_REQUEST['nome'] . "%'";
    }
    
    if($_REQUEST['categoria_id'] != ''){
    	$sql.= "      AND p.categoria_id = " . $_REQUEST['categoria_id'];
    }
    
    if($_REQUEST['marca_id'] != ''){
    	$sql.= "      AND p.marca_id = " . $_REQUEST['marca_id'];
    }
	    ...
	    <table>
	        ...
	        ?>
		        <tr <?php echo ($i % 2 == 0) ? 'class="altrow"' : 'class=""'; ?>>
		            <td style='width: 20%;'>
		            	<img src='../img/produtos/<?php echo $row->imagem?>' style='width: 60%;'>
		            </td>
